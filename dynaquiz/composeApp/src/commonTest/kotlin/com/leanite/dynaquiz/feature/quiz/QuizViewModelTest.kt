...
class QuizViewModelTest {
    ...
    private fun question(id: String = "q-1", statement: String = "Q?", options: List<String> = listOf("A", "B")) =
    ...
    @Test
    fun `Started should start countdown at 3 and decrement each second to one`() = runTest {
    ...
    @Test
    fun `Started with successful prefetch should transition to Playing with the first question`() = runTest {
    ...
    @Test
    fun `Started with failure on first question should emit QuestionLoadFailed and NavigateBack`() = runTest {
    ...
    @Test
    fun `Playing in Timed mode should initialize timeRemainingSec with perQuestionSeconds`() = runTest {
    ...
    @Test
    fun `timer should decrement each second during Playing`() = runTest {
        stubQuestionsSequentially(question("q-1"), question("q-2"), question("q-3"))
        val viewModel = createViewModel(ChallengeMode.Timed.Hard)
    ...
    @Test
    fun `timer reaching zero should register a TimedOut log and advance to the next question`() = runTest {
    ...
    @Test
    fun `Playing in Relaxed mode should keep timeRemainingSec null`() = runTest {
    ...
    @Test
    fun `AnswerSelected should mark selectedAnswer and isSubmitting immediately`() = runTest {
        stubQuestionsSequentially(question(), question("q-2"))
    ...
    @Test
    fun `AnswerSelected while isSubmitting should be ignored and not trigger a second submit`() = runTest {
        stubQuestionsSequentially(question(), question("q-2"))
        stubAlwaysCorrect()
        val viewModel = createViewModel(ChallengeMode.Relaxed)
    ...
    @Test
    fun `AnswerSelected with Success Confirmed correct should register log Confirmed correct true`() = runTest {
        stubQuestionsSequentially(question(), question("q-2"))
        stubAlwaysCorrect()
        val viewModel = createViewModel(ChallengeMode.Relaxed)
        ...
        val lastLog = viewModel.uiState.value.answerLog.last()
        assertEquals(AnswerOutcome.Confirmed(correct = true), lastLog.outcome)
    }

    @Test
    fun `AnswerSelected with Success Confirmed incorrect should register log Confirmed correct false`() = runTest {
        stubQuestionsSequentially(question(), question("q-2"))
        everySuspend { quizRepository.submitAnswer(any(), any()) } returns
    ...
    @Test
    fun `AnswerSelected with Error should emit AnswerSubmitFailed and register outcome SubmitFailed`() = runTest {
        stubQuestionsSequentially(question(), question("q-2"))
        everySuspend { quizRepository.submitAnswer(any(), any()) } returns
        ...
        assertEquals(AnswerOutcome.SubmitFailed, viewModel.uiState.value.answerLog.last().outcome)
    }

    // ----- Prefetch -----

    @Test
    fun `entering Playing should prefetch the next question`() = runTest {
        ...
        verifySuspend(dev.mokkery.verify.VerifyMode.exactly(2)) { quizRepository.getRandomQuestion() }
    }

    @Test
    fun `prefetch should not be scheduled on the last question`() = runTest {
        ...
        verifySuspend(dev.mokkery.verify.VerifyMode.exactly(QuizRules.TOTAL_QUESTIONS)) {
    ...
    @Test
    fun `answering the tenth question should mark phase Completed and currentQuestionIndex equal to total`() = runTest {
        val questions = (1..QuizRules.TOTAL_QUESTIONS).map { question("q-$it") }
        stubQuestionsSequentially(*questions.toTypedArray())
        stubAlwaysCorrect()
        val viewModel = createViewModel(ChallengeMode.Relaxed)
    ...
    @Test
    fun `completion should call SaveQuizSessionUseCase with score computed from the answerLog`() = runTest {
        val questions = (1..QuizRules.TOTAL_QUESTIONS).map { question("q-$it") }
        stubQuestionsSequentially(*questions.toTypedArray())
        stubAlwaysCorrect()
        val viewModel = createViewModel(ChallengeMode.Relaxed)
    ...
    @Test
    fun `completion should emit NavigateToResult with the QuizSessionResult`() = runTest {
        val questions = (1..QuizRules.TOTAL_QUESTIONS).map { question("q-$it") }
        stubQuestionsSequentially(*questions.toTypedArray())
        stubAlwaysCorrect()
        val viewModel = createViewModel(ChallengeMode.Relaxed)
    ...
    @Test
    fun `BackPressed should open the exit dialog`() = runTest {
        stubQuestionsSequentially(question(), question("q-2"))
        val viewModel = createViewModel(ChallengeMode.Relaxed)
    ...
    @Test
    fun `ExitCancelled should close the dialog keeping the running phase`() = runTest {
        stubQuestionsSequentially(question(), question("q-2"))
        val viewModel = createViewModel(ChallengeMode.Relaxed)
    ...
    @Test
    fun `ExitConfirmed should close the dialog and emit NavigateBack`() = runTest {
        stubQuestionsSequentially(question(), question("q-2"))
        val viewModel = createViewModel(ChallengeMode.Relaxed)
