...
class QuizRepositoryImplTest {
    ...
    private fun createRepository() = QuizRepositoryImpl(
        remoteDataSource = remoteDataSource,
        ioDispatcher = testDispatcher,
    )

    @Test
    fun `getRandomQuestion should return Success with mapped Question when remote responds`() = runTest {
        everySuspend { remoteDataSource.fetchRandomQuestion() } returns QuestionDTO(
            id = "q-42",
            statement = "Capital?",
    ...
    @Test
    fun `getRandomQuestion should return Error NoInternet when remote throws IOException`() = runTest {
    ...
    @Test
    fun `getRandomQuestion should return Error Unknown when remote throws unmapped Throwable`() = runTest {
    ...
    @Test
    fun `submitAnswer should forward unwrapped questionId value and answer to remote`() = runTest {
    ...
    @Test
    fun `submitAnswer should return Success with mapped Answer when remote responds`() = runTest {
        everySuspend { remoteDataSource.submitAnswer(any(), any()) } returns AnswerResultDTO(result = true)
    ...
    @Test
    fun `submitAnswer should return Error NoInternet when remote throws IOException`() = runTest {
    ...
    @Test
    fun `warmupServer should swallow any Throwable without propagating`() = runTest {
        everySuspend { remoteDataSource.fetchRandomQuestion() } throws RuntimeException("boom")
    ...
    @Test
    fun `warmupServer should call fetchRandomQuestion exactly once`() = runTest {
        everySuspend { remoteDataSource.fetchRandomQuestion() } returns QuestionDTO(
            id = "q-1", statement = "", options = emptyList(),
        )
